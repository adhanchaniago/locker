        ...
        <div class="col s12 m3">
          ...
          <ul class="collection with-header z-depth-1">
            ...
            <?php
              ...
              $result = mysqli_query($conn, $sql);
              $row = mysqli_fetch_array($result);
              echo "<li class='collection-item'>Total Karyawan: <span class='secondary-content green-text'>".$row['total']."</span></li>";

              // Total
              $sql = "SELECT COUNT(record_id) as total from `record` WHERE record_status='approved'";
              $result = mysqli_query($conn, $sql);
              $row = mysqli_fetch_array($result);
              echo "<li class='collection-item'>Paid Karyawan: <span class='secondary-content green-text'>".$row['total']."</span></li>";

              // total paid locker
              $sql = "SELECT SUM(record_price) as price from `record` WHERE record_status='approved'";
              $result = mysqli_query($conn, $sql);
              $row = mysqli_fetch_array($result);
              echo "<li class='collection-item'>Total paid locker: <span class='secondary-content green-text'>IDR".$row['price']."</span></li>";
            ?>
          </ul>
        </div>
      </div>
