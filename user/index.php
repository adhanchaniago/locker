  ...
  <section class="section">
    <div class="container2">
      ...
      <ul class="collapsible">
		...
		<li>
          <div class="collapsible-header active blue darken-2 white-text">
            <i class="fas fa-wrench"></i>&nbsp Status Pengajuan Perbaikan Locker
          </div>
          <div class="collapsible-body">
            <table class="responsive-table highlight centered">
              <thead>
              ...
              <tbody>
                <?php
                  $i = 1;
                  $sql = "SELECT * FROM `maintenance_form` WHERE nik='".$_SESSION['s_id']."'";
                  $result = mysqli_query($conn, $sql);
                  while ($row = mysqli_fetch_array($result)):
                ?>
                <tr>
                  ...
                  <td><?php
							  ...
							  $maintenance_id=$row['maintenance_id'];
								$sql = "SELECT * FROM maintenance_form WHERE nik='$nik' and maintenance_id='$maintenance_id'";
								$result = mysqli_query($conn, $sql);
								$row = mysqli_fetch_array($result);
								$pengajuan = $row['permohonan'];
								$pengechekan = $row['hasil_chek'];
								$perbaikan = $row['tindakan'];
								if ($pengechekan == "" and $perbaikan=="")
									echo '<p style="color:blue">PENGAJUAN</p>';
								elseif ($pengechekan != "" and $perbaikan=="")
									echo '<p style="color:yellow">PENGECHEKAN</p>';
								
								else
									echo '<p style="color:green">SELESAI DIPERBAIKI</p>';
								?></td>
                </tr>
              <?php endwhile ?>
              </tbody>
