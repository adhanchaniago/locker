  ...
  if (filter_has_var(INPUT_POST, 'submit')){
    ...
    if (!empty($id) && !empty($username) && !empty($name) && !empty($email) && !empty($phone) && !empty($password)){
      ...
      if (filter_var($email, FILTER_VALIDATE_EMAIL) === false){
        ...
        $sql = "UPDATE `student` SET `student_id`='$id', `student_username`='$username', `student_pwd`='$hashedPwd', `student_department`='$code_bagian', `student_name`='$name', `student_email`='$email', `student_phone`='$phone' WHERE student_id='$id'";
              ...
              <div class="row">
                ...
                <select name="code_bagian"  id="code_bagian">
						<option>Pilih Bagian</option>
										...
										?>
										<?php if($rlocker['code_bagian'] ==$row['student_department'] ):?>
